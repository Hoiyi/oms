...
; + function($) {
    ...
    UI_choose.prototype = {
        ...
        _setHtml_select: function() {
            ...
            this.el.find('option').each(function(index, el) {
                var _this = $(el),
                    ...
                    _value = typeof(_this.attr('data-code')) == 'undefined'?_value:_this.attr('data-code'),
        ...
        // 组建并获取相关的dom元素-select;
        _addNode: function(value,name) {
        	var _option = '<option value="'+value+'">"'+name+'"</option>';
        	var  _ohtml = '<li title="' + name + '" data-value="' + value + '" class="">' + name + '</li> ';
        	console.log(this.el)
        	$(this.el).append(_option);
        	$(this._wrap).append(_ohtml);
        },
        // 绑定事件；
        _bindEvent: function() {
            var _this = this;
            _this._wrap.on('click', 'li', function() {
            	 if(_this._opt.disabled){
             		return;
             		}
                var _self = $(this);
                if (_self.hasClass('disabled'))
                    return;
                if (_self.hasClass('filter-item'))
                	return;
        ...
        _un_select: function(value) {
         
            this._wrap.children('li[data-value="' + value + '"]').removeClass(this._opt.active);
              
            return this;
        },

        _selected: function() {
        	var selectItens = this._wrap.children('.selected')
        	var itemsId = '';
        	
        	selectItens.each(function(){
        		itemsId += $(this).attr("data-value") + ",";
        	});
        	
        	itemsId = itemsId.substr(0,itemsId.length-1)
        	console.log(itemsId);
            return itemsId;
        },
        // 获取或设置值:ul
        ...
        select:function(value){
        	this._val_select(value);
        },
        selectFirst:function(){
        	this._val_select($(this._items[0]).attr('data-value'));
        },
        addNode:function(val,name){
        	this._addNode(val,name);
        },
        unSelect:function(value){
        	this._un_select(value);
        },
        selected:function(){
        ...
        disabled:function(){
         	this._opt.disabled = true;
       },
       enabled:function(){
    	   this._opt.disabled = false;
      },
...
//    	  this._wrap.children('li[data-value="' + value + '"]').removeClass(this._opt.active);
    	  ...
    	  $(nodes).each(function(){
    		  _this._wrap.children('li[data-value="' + this + '"]').addClass("disabled");
    	  });
      },
        // 点击事件；
